"""Retrain models using all available historical data"""
import sys
...
def main():
    logger.info("=" * 80)
    logger.info("RETRAINING WITH FULL HISTORICAL DATA")
    logger.info("=" * 80)
    logger.info("This will retrain models for diseases with extensive historical data")
    logger.info("(Measles and Yellow Fever have 10+ years of data)")
    logger.info("=" * 80)
    ...
    # Focus on diseases with extensive historical data
    diseases = ['Measles', 'Yellow Fever']

    successful = 0
    failed = 0
    total = len(locations) * len(diseases)

    logger.info(f"\nRetraining {len(diseases)} diseases across {len(locations)} districts")
    ...
    for disease in diseases:
        ...
        for location_name, location_uid in locations:
            logger.info(f"\n[{successful + failed + 1}/{total}] {disease} in {location_name} ({location_uid})")
            logger.info('-'*80)

            try:
                # Train with ALL available data (start_date=None by default now)
                logger.info("Training model with full historical data...")
                result = service.train_model(disease, location_uid)

                if result['success']:
                    logger.info(f"✓ Training successful!")
                    logger.info(f"  Training: {result['training_samples']} samples (ALL available data)")
                    ...
                    # Generate forecast
                    logger.info("Generating forecast with new model...")
