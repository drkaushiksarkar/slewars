"""
Automatic Climate Data Loader (Non-Interactive)
Loads climate data from ZIP files in data/ folder into PostgreSQL database
This version runs non-interactively for automated setup
"""
...
def check_existing_data():
    """Check if climate data already exists in database"""
    try:
...
def main():
    """Main function to process all GRIB files (non-interactive)"""
    logger.info("=" * 60)
    logger.info("Automatic Climate Data Loader")
    logger.info("=" * 60)

    # Check if data already exists
    existing_count = check_existing_data()
    if existing_count > 0:
        logger.info(f"Found {existing_count} existing ERA5 records in database")
        logger.info("Skipping data load. To force reload, delete existing data first.")
        logger.info("=" * 60)
        return 0
    ...
    if not zip_files:
        ...
        logger.info("Climate data will be downloaded when needed")
        logger.info("=" * 60)
        return 0
    ...
    logger.info("\n" + "=" * 60)
    logger.info("✓ Climate Data Loading Complete!")
    logger.info("=" * 60)
