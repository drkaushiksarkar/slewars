...
interface ERA5Config {
  apiKey: string;
  apiUrl: string;
  sierraLeoneArea: number[]; // [north, west, south, east]
}

class ClimateService {
  private era5Config: ERA5Config;
   ...
   */
  private findMissingDates(
    cachedData: ClimateData[],
    startDate: string,
    endDate: string
  ): string[] {
   ...
   */
  private async getLocationCoordinates(locationUid: string): Promise<{lat: number, lon: number} | null> {
    try {
      const query = `
        SELECT
   ...
   */
  private async storeCachedData(data: ClimateData[]): Promise<void> {
   ...
   */
  async getAggregatedClimateData(
    locationUid: string,
    startDate: string,
    endDate: string,
   ...
   */
  async getClimateStatistics(locationUid: string): Promise<any> {
    try {
      const query = `
        SELECT
          location_uid,
   ...
   */
  async syncAllDistricts(startDate: string, endDate: string): Promise<void> {
    try {
