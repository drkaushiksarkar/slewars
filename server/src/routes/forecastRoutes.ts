 ...
 */
router.get('/anomaly-detection/:disease', async (req: Request, res: Response) => {
  try {
    ...
    const result = await forecastService.detectAnomalies(
      disease,
      parseInt(level as string),
      start_date as string,
      end_date as string,
      location_uid as string | undefined
    );
 ...
 */
router.get('/:disease/:locationUid', async (req: Request, res: Response) => {
  try {
...
router.get('/districts', async (req: Request, res: Response) => {
  try {
    ...
    const districts = await forecastService.getDistrictsWithForecasts(
 ...
 */
router.get('/risk-analysis/:disease', async (req: Request, res: Response) => {
  try {
    const { disease } = req.params;
