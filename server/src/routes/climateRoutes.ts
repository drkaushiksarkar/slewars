 ...
 */
router.get('/:locationUid', async (req: Request, res: Response) => {
  try {
    ...
    if (aggregation === 'week' || aggregation === 'month') {
      data = await climateService.getAggregatedClimateData(
        locationUid,
        startDate as string,
        endDate as string,
        aggregation as 'week' | 'month'
      );
    } else {
      data = await climateService.getClimateData(
        locationUid,
        startDate as string,
        endDate as string
      );
 ...
 */
router.get('/:locationUid/statistics', async (req: Request, res: Response) => {
  try {
    const { locationUid } = req.params;
