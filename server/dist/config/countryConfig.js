...
const countrySchema = z.record(z.string(), z.object({
    id: z.string(),
    name: z.string(),
    flag: z.string(),
...
export const getCountryConfigs = async () => {
    if (!cache) {
        try {
            ...
            logger.info({ countryCount: Object.keys(cache).length }, "Loaded country configuration");
        }
        catch (error) {
...
export const getCountryConfig = async (countryId) => {
