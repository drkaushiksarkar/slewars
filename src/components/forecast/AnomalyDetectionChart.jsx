...
const AnomalyDetectionChart = ({
  data,
  forecast,
  isIsolationForestExpanded,
  setIsIsolationForestExpanded,
  isSTLExpanded,
  setIsSTLExpanded
}) => {
  ...
  if (!data || !data.time_series || data.time_series.length === 0) {
    return <EmptyState type="no-data" className="py-8" />;
  }

  const { time_series, thresholds, anomalies, summary, stl_decomposition } = data;
  ...
  // Group data by location for multi-line chart and include STL components
  const chartData = useMemo(() => {
    // Get unique dates
    const dates = [...new Set(time_series.map(d => d.date))].sort();

    // For each date, create an object with all locations' cases and STL components
    return dates.map((date, idx) => {
      ...
      // Add STL decomposition components if available
      if (stl_decomposition && idx < stl_decomposition.dates.length) {
        dataPoint.trend = stl_decomposition.trend[idx];
        dataPoint.seasonal = stl_decomposition.seasonal[idx];
        dataPoint.residual = stl_decomposition.residual[idx];
        dataPoint.observed_stl = stl_decomposition.observed[idx];
      }

      return dataPoint;
    });
  }, [time_series, stl_decomposition]);
  ...
  return (
    <div className="space-y-4">
      {/* Time Series Chart - Enhanced with gradient fills and layer selection */}
      <div
        className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-4 border-2 border-gray-200 shadow-lg"
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-xs text-red-600 font-medium">{summary.total_anomalies}</span>
                <span className="text-xs text-red-500">anomalies</span>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-emerald-500">cases</span>
              </div>
            </div>
        ...
        {stl_decomposition && (
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span className="text-xs font-semibold text-gray-700 mr-2">Layers:</span>
            <button
              onClick={() => toggleLayer('observed')}
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
                visibleLayers.observed
            ...
            <button
              onClick={() => toggleLayer('seasonal')}
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
                visibleLayers.seasonal
            ...
            <button
              onClick={() => toggleLayer('residual')}
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
                visibleLayers.residual
        ...
        <ResponsiveContainer width="100%" height={350}>
          <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
            ...
            {/* STL Components - Overlaid layers */}
            {stl_decomposition && visibleLayers.trend && (
              <Line
                type="monotone"
                dataKey="trend"
                stroke="#10b981"
            ...
            {stl_decomposition && visibleLayers.seasonal && (
              <Line
                type="monotone"
                dataKey="seasonal"
                stroke="#8b5cf6"
            ...
            {stl_decomposition && visibleLayers.residual && (
              <Scatter
                dataKey="residual"
                fill="#f97316"
            ...
            {/* Area and Line for each location - Only show if observed layer is enabled */}
            {(!stl_decomposition || visibleLayers.observed) && locations.map((location, idx) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col">
          ...
          <div className="space-y-2 overflow-y-auto pr-2 custom-scrollbar max-h-[350px]">
            ...
            <div className="relative overflow-hidden p-3 rounded-xl border-2 bg-gradient-to-r from-indigo-50 to-blue-50 border-indigo-200 transition-all duration-200 hover:shadow-md">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <div className="flex items-center gap-1 bg-green-100 text-green-700 text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0">
                      <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div className="flex items-center gap-3 text-xs">
                  <span className="text-gray-700">
                    Anomaly Rate: <span className="font-bold text-indigo-700">{(summary.anomaly_rate * 100).toFixed(1)}%</span>
                  </span>
                  <span className="text-gray-600">
                    Contamination: <span className="font-semibold">{summary.contamination ? (summary.contamination * 100).toFixed(0) : '10'}%</span>
                  </span>
                </div>
                <motion.div
                  initial={false}
                  animate={{
                    height: isIsolationForestExpanded ? "auto" : 0,
                    opacity: isIsolationForestExpanded ? 1 : 0
                  }}
                  transition={{ duration: 0.3, ease: "easeInOut" }}
                  style={{ overflow: "hidden" }}
                >
                  <p className="text-xs text-gray-700 leading-relaxed mt-3 pt-3 border-t border-indigo-200">
                    Advanced machine learning algorithm that identifies anomalies by isolating outliers in multi-dimensional feature space. Analyzes temporal patterns and statistical trends.
                  </p>
                </motion.div>
              </div>
              <button
                onClick={() => setIsIsolationForestExpanded(!isIsolationForestExpanded)}
                className="flex-shrink-0 rounded-lg bg-indigo-600 hover:bg-indigo-700 flex items-center justify-center gap-1 px-2.5 py-1.5 transition-colors shadow-sm"
                title={isIsolationForestExpanded ? "Show Less" : "More Info"}
              >
                <span className="text-xs font-semibold text-white">
                  {isIsolationForestExpanded ? "Less" : "Info"}
                </span>
                <motion.svg
                  animate={{ rotate: isIsolationForestExpanded ? 180 : 0 }}
            ...
            {/* STL Decomposition */}
            {stl_decomposition && (
              <div className="relative overflow-hidden p-3 rounded-xl border-2 bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200 transition-all duration-200 hover:shadow-md">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      <span className="text-gray-700">
                        Seasonal: <span className="font-bold text-purple-700">{(stl_decomposition.seasonal_strength * 100).toFixed(1)}%</span>
                      </span>
                      <span className="text-gray-600">
                        Trend: <span className="font-semibold">{(stl_decomposition.trend_strength * 100).toFixed(1)}%</span>
                      </span>
                    </div>
                    <motion.div
                      initial={false}
                      animate={{
                        height: isSTLExpanded ? "auto" : 0,
                        opacity: isSTLExpanded ? 1 : 0
                      }}
                      transition={{ duration: 0.3, ease: "easeInOut" }}
                      style={{ overflow: "hidden" }}
                    >
                      <p className="text-xs text-gray-700 leading-relaxed mt-3 pt-3 border-t border-purple-200">
                        Decomposes time series into <span className="font-medium text-green-600">Trend</span> (long-term direction), <span className="font-medium text-purple-600">Seasonal</span> (repeating patterns), and <span className="font-medium text-orange-600">Residual</span> (random noise) components.
                      </p>
                    </motion.div>
                  </div>
                  <button
                    onClick={() => setIsSTLExpanded(!isSTLExpanded)}
                    className="flex-shrink-0 rounded-lg bg-purple-600 hover:bg-purple-700 flex items-center justify-center gap-1 px-2.5 py-1.5 transition-colors shadow-sm"
                    title={isSTLExpanded ? "Show Less" : "More Info"}
                  >
                    <span className="text-xs font-semibold text-white">
                      {isSTLExpanded ? "Less" : "Info"}
                    </span>
                    <motion.svg
                      animate={{ rotate: isSTLExpanded ? 180 : 0 }}
                      transition={{ duration: 0.3 }}
                      className="w-3.5 h-3.5 text-white"
                      fill="none"
                      stroke="currentColor"
